...
export default function CodeNameResubmitBanner({ rejectionReason }: { rejectionReason: string | null }) {
  ...
  async function handleSubmit() {
    ...
    const res = await fetch('/api/player/team', {
      ...
      body: JSON.stringify({ action: 'resubmit_code_name', code_name: value }),
    })
  ...
  if (done) {
    return (
      <div className="rounded-xl border border-zinc-700 bg-zinc-900 p-4 text-sm text-zinc-300">
        Code name resubmitted — pending admin approval.
      </div>
    )
  }
  ...
  return (
    <div className="rounded-xl border border-red-800 bg-red-950/20 p-4 space-y-3">
      <div>
        <div className="text-red-400 font-semibold text-sm">Your code name was rejected</div>
        {rejectionReason && (
